...
<div class="container py-2">
    ...
    @else
        <form action="{{ route('patrocinadores.update', $patrocinador->id) }}" method="POST" enctype="multipart/form-data" class="card shadow-sm">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            <input type="hidden" name="id_evento" value="{{ Request::session()->get('evento.id') }}" />
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title m-0">Editar Patrocinador</h5>
            </div>
            <div class="card-body">
                <div class="form-row">
                ...
                <div class="form-row">
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label>Nome:</label>
                            <input type="text" class="form-control @error('nm_patrocinador') is-invalid @enderror" name="nm_patrocinador" value="{{ $patrocinador->nm_patrocinador }}" required />
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label>Site:</label>
                            <input type="text" class="form-control @error('lk_site') is-invalid @enderror" name="lk_site" value="{{ $patrocinador->lk_site }}" />
                            @error('lk_site')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label>Posição:</label>
                            <input type="number" class="form-control @error('posicao') is-invalid @enderror" name="posicao" value="{{ $patrocinador->posicao }}" />
                            @error('posicao')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
