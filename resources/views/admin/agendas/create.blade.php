...
<div class="container py-2">
    ...
    @else
        <form action="{{ route('agendas.store') }}" method="POST" enctype="multipart/form-data" class="card shadow-sm">
            {{ csrf_field() }}
            <input type="hidden" name="id_evento" value="{{ Request::session()->get('evento.id') }}" />
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title m-0">Novo agendamento</h5>
            </div>
            <div class="card-body">
                <div class="form-row">
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label>Nome:</label>
                            <input type="text" class="form-control @error('nm_agenda') is-invalid @enderror" name="nm_agenda" value="{{ old('nm_agenda') }}" required />
                            @error('nm_agenda')
                ...
                <div class="form-row">
                    <div class="col-12 col-md-3">
                        <div class="form-group">
                            ...
                            <calendar-component name="dt_inicio" value="{{ (old('dt_inicio')) ? \Carbon\Carbon::createFromFormat('d/m/Y', old('dt_inicio'))->toDateString() : '' }}" />
                            @error('dt_inicio')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
